...
#######################################
  ...
  # inst.plot3: GRAD vs UGRAD avg quiz score — lollipop chart
  output$inst.plot3 = renderPlot({
    ggplot(grad_ugrad_scores, aes(x = factor(QUIZ_NUMBER), y = avg_score, color = PROG, group = PROG)) +
  ...
  # inst.plot4: Skill survey distribution — density + rug
  output$inst.plot4 = renderPlot({
    avg_skill <- mean(skill_dist$SKILL_SURVEY_SCORE, na.rm = TRUE)
    ggplot(skill_dist, aes(x = SKILL_SURVEY_SCORE)) +
      geom_histogram(aes(y = after_stat(density)), binwidth = 2,
                     fill = cornell_navy, color = "white", alpha = 0.75) +
      geom_density(color = cornell_red, linewidth = 1.1, adjust = 1.2) +
      geom_rug(color = cornell_gray, alpha = 0.4, length = unit(0.03, "npc")) +
      geom_vline(xintercept = avg_skill, linetype = "dashed",
                 color = cornell_red, linewidth = 0.9) +
      annotate("text", x = avg_skill + 0.5, y = Inf,
               label = paste0("Mean: ", round(avg_skill, 1)),
               hjust = 0, vjust = 1.5, color = cornell_red,
               size = 3.5, fontface = "bold") +
      labs(x = "Skill Survey Total Score (0–35)", y = "Density") +
      theme_cornell()
  })
  ...
  output$stu.plot1 = renderPlot({
    ...
    ggplot(plot_data, aes(x = QUIZ_NUMBER)) +
      geom_ribbon(aes(ymin = pmin(my_score, class_avg),
                      ymax = pmax(my_score, class_avg)),
                  fill = cornell_red, alpha = 0.08, na.rm = TRUE) +
  ...
  output$stu.plot2 = renderPlot({
    stu_comp <- stu_sessions() %>%
      ...
      mutate(above_avg = completion_rate >= class_completion)
